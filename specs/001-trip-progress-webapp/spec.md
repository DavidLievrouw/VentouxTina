...
- Q: Moet een logregel een datum of timestamp bevatten? → A: Een logregel moet een timestamp bevatten.
- Q: Moeten DB-wijzigingen onmiddellijk zichtbaar zijn? → A: Niet verplicht; zichtbaarheid na cache-expiratie is aanvaardbaar.
...
Als bezoeker wil ik een logboek zien met timestamp, kilometers en activiteit (lopen, fietsen, wandelen),
...
1. **Given** geldige logrecords in MariaDB, **When** de pagina laadt, **Then** toont het logboek per regel timestamp, afstand in kilometers en activiteit.
...
- **FR-013**: Wijzigingen in de database MUST zichtbaar worden na herladen van de pagina of na cache-expiratie (maximaal 1 minuut), inclusief aangepaste kaartvoortgang en percentage.
...
- **FR-021**: Er MUST een eenmalig PowerShell-script aanwezig zijn dat `TripRoute`, checkpoints, `FundraisingGoal` en `ProjectContext` initieel vult.
- **FR-022**: Het seedscript MUST route- en checkpointdata ophalen via OpenRouteService (gratis tier).
...
- **CA-004c Seeder Prerequisite**: Route/default seeding wordt uitgevoerd als prerequisite vóór story-uitvoering.
...
- **TripLogEntry**: Een logregel met timestamp, afgelegde kilometers en activiteitstype.
...
- `TripRoute`, checkpoints, `FundraisingGoal` en `ProjectContext` worden initieel gevuld via een eenmalig PowerShell-seedscript.
